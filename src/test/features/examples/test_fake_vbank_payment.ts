...
export async function test_fake_vbank_payment
    (connector: imp.IamportConnector): Promise<IIamportVBankPayment>
{
    // 가상 결제 발행하기
...
async function issue(connector: imp.IamportConnector): Promise<IIamportVBankPayment>
{
    /**
     * 가상 계좌로 결제 요청하기.
     * 
     ...
     */
    const output: IIamportResponse<IIamportVBankPayment> = await imp.functional.vbanks.store
    (
        await connector.get(),
        {
            merchant_uid: v4(),
    ...
    if (webhook.imp_uid !== output.response.imp_uid)
        throw new Error("Bug on vbanks.store(): failed to deliver the webhook event.");
    ...
    ): Promise<IIamportVBankPayment>
{
    ...
    if (webhook.imp_uid !== ready.imp_uid)
        throw new Error("Bug on internal.deposit(): failed to deliver the webhook event.");
