...
export async function test_fake_certification
    (connector: imp.IamportConnector): Promise<void>
{
    /**
     * 본인인증 요청 시뮬레이션.
     * 
     ...
     */
    const accessor: IIamportResponse<IIamportCertification.IAccessor> = await imp.functional.certifications.otp.request
    (
        await connector.get(),
        {
            name: "남정호",
     ...
     */
    const uncertified: IIamportResponse<IIamportCertification> = await imp.functional.certifications.at
    (
        await connector.get(), 
        accessor.response.imp_uid
    );
    if (uncertified.response.certified === true)
        throw new Error("Bug on certifications.otp.request(): must not be certified yet.");
     ...
     * 동일한 바이다.
     */
    const confirmed: IIamportResponse<IIamportCertification> = await imp.functional.certifications.otp.confirm
    (
        await connector.get(),
        accessor.response.imp_uid,
        {
    ...
    if (confirmed.response.certified === false)
        throw new Error("Bug on certifications.otp.confirm(): must be certified.");
}