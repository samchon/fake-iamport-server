...
export async function test_fake_card_payment
    (connector: imp.IamportConnector): Promise<IIamportCardPayment>
{
    /**
     * 결제 요청 레코드 발행하기.
     * 
     ...
     */ 
    const output: IIamportResponse<IIamportCardPayment> = await imp.functional.subscribe.payments.onetime
    (
        await connector.get(),
        {
