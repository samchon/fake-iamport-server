    ...
    (connector: imp.IamportConnector): Promise<void>
{
     ...
     */
    const onetime: IIamportResponse<IIamportCardPayment> = await imp.functional.subscribe.payments.onetime
    (
        await connector.get(),
        {
            customer_uid,
     ...
     */
    const again: IIamportResponse<IIamportCardPayment> = await imp.functional.subscribe.payments.again
    (
        await connector.get(),
        {
            customer_uid,
    ...
    ): Promise<IIamportCardPayment>
{
    ...
    if (webhook.imp_uid !== imp_uid)
        throw new Error("Bug on subscribe.payments.onetime(): failed to deliver the webhook event.");
    else if (webhook.status !== "paid")
        throw new Error("Bug on subscribe.payments.onetime(): its status must be paid.");
